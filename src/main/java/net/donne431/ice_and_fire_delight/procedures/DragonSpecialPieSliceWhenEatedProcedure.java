...
public class DragonSpecialPieSliceWhenEatedProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity) {
		...
		if (CommonConfigConfiguration.ENABLE_EXPLOSIONS_WHEN_EAT_SPECIAL_PIE_SLICE.get() == true) {
			if (Math.random() < (double) CommonConfigConfiguration.CHANCE_EXPLOSIONS_WHEN_EAT_SPECIAL_PIE_SLICE.get()) {
				if (((LivingEntity) entity).getAttribute(IceAndFireDelightModAttributes.NUMBEROFEXPLOSIONS.get()).getValue() <= (double) CommonConfigConfiguration.NUMBER_DEATH_BEFORE_EXPLOSIONS_STOP_WHEN_EATING_SPECIAL_PIE_SLICE.get() - 1) {
					if (entity instanceof ServerPlayer _player) {
